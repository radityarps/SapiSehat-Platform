...
Replace active backend and Flutter inference with the verified MobileNetV3 model whose ordered outputs are `FMD`, `healthy`, and `non_cattle`, while keeping the existing scan, sync, history, evidence, and dashboard flows substantially unchanged.
...
- exact class order and preprocessing contract;
...
- Validate tensor count, shape, dtype, finite probabilities, probability range/sum, and exact output order before readiness.
...
- Use the same input preprocessing and output order as the backend, with no fallback class order or model version.
...
- Backend and Flutter both map indexes as `0=FMD`, `1=healthy`, `2=non_cattle`.
