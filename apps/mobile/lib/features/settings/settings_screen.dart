        ...
        children: [
                  ...
                  Row(
                    children: [
                      CircleAvatar(
                        ...
                        child: Text(
                          session.name.isNotEmpty ? session.name[0].toUpperCase() : 'P',
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(session.name.isEmpty ? 'Demo Farmer' : session.name,
                                style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                            Text(session.email, style: const TextStyle(color: Colors.grey, fontSize: 13)),
                            if (session.address != null && session.address!.isNotEmpty)
                              Text(session.address!, style: const TextStyle(color: Colors.grey, fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                            Text(session.jurisdictionId, style: const TextStyle(color: Colors.grey, fontSize: 12)),
                          ],
                        ),
                      ),
                    ],
          ...
          FutureBuilder<FarmerPreferences>(
            ...
            builder: (context, snapshot) {
              final prefs = snapshot.data ?? FarmerPreferences(farmerId: session.farmerId);
              return Card(
          ...
          const _InfoCard(icon: Icons.location_on, title: 'Izin lokasi', body: 'Dipakai untuk mengisi alamat dari GPS. Alamat tetap perlu dikonfirmasi.'),
