...
type JurisdictionItem = { id: string; name: string; level: string; parent_id: string | null };

const ROLES = ['admin', 'province_officer', 'district_officer', 'village_officer', 'viewer'];

async function fetchUsers(token: string, agencyUserId: string): Promise<{ users: AgencyUserItem[] }> {
...
async function fetchJurisdictions(token: string, agencyUserId: string): Promise<{ jurisdictions: JurisdictionItem[] }> {
...
async function createUser(token: string, agencyUserId: string, data: AgencyUserItem) {
  const res = await fetch('/api/agency/users', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}`, 'X-Agency-User-Id': agencyUserId },
...
async function updateUser(token: string, agencyUserId: string, userId: string, data: { role: string; jurisdiction_id?: string }) {
...
export function UsersClient() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agency-users'] }); resetForm(); },
  });

  const updateMutation = useMutation({
    mutationFn: (data: { userId: string; role: string; jurisdiction_id: string }) =>
      updateUser(token, agencyUserId, data.userId, { role: data.role, jurisdiction_id: data.jurisdiction_id }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agency-users'] }); resetForm(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (userId: string) => deleteUser(token, agencyUserId, userId),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['agency-users'] }); },
  });
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editingUser) {
      updateMutation.mutate({ userId: editingUser.id, role: formRole, jurisdiction_id: formJurisdiction });
    } else {
      createMutation.mutate({ id: formId, role: formRole, jurisdiction_id: formJurisdiction });
    }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-md border bg-card p-4 space-y-3">
          <p className="text-sm font-medium">{editingUser ? `Edit: ${editingUser.id}` : 'New agency user'}</p>
          {!editingUser && (
            <div className="space-y-1">
              <Label htmlFor="user-id">User ID</Label>
              <Input id="user-id" value={formId} onChange={(e) => setFormId(e.target.value)} placeholder="e.g. agency-6" required />
            </div>
          )}
          <div className="space-y-1">
      ...
      <div className="rounded-md border">
        <table className="w-full text-sm">
          ...
          <tbody>
                ...
                <td className="px-4 py-2 text-right">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <Button
                      ...
                      onClick={() => { if (confirm(`Delete user ${user.id}?`)) deleteMutation.mutate(user.id); }}
                    >
            ...
            {users.length === 0 && (
              <tr><td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">No users found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
