...
import { clearAgencyToken, getAgencyToken, sessionEventName, setAgencyToken } from '@/src/shared/auth/session';
...
export function AgencySessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const signIn = useCallback(async (nextToken: string, nextAgency?: AgencyMe | null) => {
...
export function useAgencySession() {
  ...
  if (!session) throw new Error('useAgencySession must be used inside AgencySessionProvider');
  return session;
}
